...
void dataPacketItem::setPointerToContent(const uchar *pkt_content, int size)
{
...
QString dataPacketItem::getLength()
{
    return QString::number(this->length);
}
...
QString dataPacketItem::getSource(){
    if(this->protocolType == 1)
        return getSrcMacAddr();
    else return getSrcIpAddr();
}
QString dataPacketItem::getDestination(){
    if(this->protocolType == 1)
...
QString dataPacketItem::getDesMacAddr(){
    ETHERNET_HEADER* ethernet;
    ethernet = (ETHERNET_HEADER*)pkt_content;
    u_char*addr;
    if(ethernet){
        addr = ethernet->destination_address;
        if(addr){
            QString res = this->byteToString(addr,1) + ":"
...
QString dataPacketItem::getSrcMacAddr(){
    ETHERNET_HEADER* ethernet;
    ethernet = (ETHERNET_HEADER*)pkt_content;
    u_char*addr;
    if(ethernet){
...
QString dataPacketItem::getDesIpAddr(){
    IP_HEADER*ip;
...
QString dataPacketItem::getSrcIpAddr(){
    IP_HEADER*ip;
    ip = (IP_HEADER*)(pkt_content + 14);
    sockaddr_in srcAddr;
...
/********************** get ethenet type **********************/
QString dataPacketItem::getMacType(){
    ETHERNET_HEADER* ethernet;
    ethernet = (ETHERNET_HEADER*)pkt_content;
    u_short ethernet_type = ntohs(ethernet->type);
...
/********************** get ip version **********************/
QString dataPacketItem::getIpVersion(){
    IP_HEADER*ip;
    ip = (IP_HEADER*)(pkt_content + 14);
    return QString::number(ip->versiosn_head_length >> 4);
}
/********************** get ip header length **********************/
QString dataPacketItem::getIpHeaderLength(){
    IP_HEADER*ip;
    ip = (IP_HEADER*)(pkt_content + 14);
    QString res = "";
...
/********************** get ip checksum **********************/
QString dataPacketItem::getIpCheckSum(){
    IP_HEADER*ip;
    ip = (IP_HEADER*)(pkt_content + 14);
...
/********************** get icmp checksum **********************/
QString dataPacketItem::getIcmpCheckSum(){
    ICMP_HEADER*icmp;
    icmp = (ICMP_HEADER*)(pkt_content + 14 + 20);
    return QString::number(ntohs(icmp->checksum),16);
}
/********************** get icmp identification **********************/
QString dataPacketItem::getIcmpIdentification(){
    ICMP_HEADER*icmp;
    icmp = (ICMP_HEADER*)(pkt_content + 14 + 20);
    return QString::number(ntohs(icmp->identification));
}
/********************** get icmp sequence **********************/
QString dataPacketItem::getIcmpSequeue(){
    ICMP_HEADER*icmp;
    icmp = (ICMP_HEADER*)(pkt_content + 14 + 20);
    return QString::number(ntohs(icmp->sequence));
}
QString dataPacketItem::getIcmpData(int size){
    char*icmp;
...
/* arp info */
QString dataPacketItem::getArpHardwareType(){
    ARP_HEADER*arp;
...
QString dataPacketItem::getArpProtocolType(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
...
/********************** get hardware length **********************/
QString dataPacketItem::getArpHardwareLength(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
    return QString::number(arp->mac_length);
}
/********************** get arp protocol length **********************/
QString dataPacketItem::getArpProtocolLength(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
    return QString::number(arp->ip_length);
}
/********************** get arp operator code **********************/
QString dataPacketItem::getArpOperationCode(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
...
QString dataPacketItem::getArpSourceEtherAddr(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
    u_char*addr;
    if(arp){
...
QString dataPacketItem::getArpDestinationEtherAddr(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
    u_char*addr;
    if(arp){
...
QString dataPacketItem::getArpSourceIpAddr(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
    if(arp){
...
QString dataPacketItem::getArpDestinationIpAddr(){
    ARP_HEADER*arp;
    arp = (ARP_HEADER*)(pkt_content + 14);
    if(arp){
...
/* tcp */
/********************** get tcp source port **********************/
QString dataPacketItem::getTcpSourcePort(){
    TCP_HEADER*tcp;
...
QString dataPacketItem::getTcpDestinationPort(){
    TCP_HEADER*tcp;
    tcp = (TCP_HEADER*)(pkt_content + 14 + 20);
...
/********************** get tcp sequence **********************/
QString dataPacketItem::getTcpSequence(){
    TCP_HEADER*tcp;
    tcp = (TCP_HEADER*)(pkt_content + 14 + 20);
    return QString::number(ntohl(tcp->sequence));
}
/********************** get tcp acknowledgment **********************/
QString dataPacketItem::getTcpAcknowledgment(){
    TCP_HEADER*tcp;
    tcp = (TCP_HEADER*)(pkt_content + 14 + 20);
    return QString::number(ntohl(tcp->ack));
}
/********************** get tcp header length **********************/
QString dataPacketItem::getTcpHeaderLength(){
    TCP_HEADER*tcp;
    tcp = (TCP_HEADER*)(pkt_content + 14 + 20);
...
QString dataPacketItem::getTcpRawHeaderLength(){
    TCP_HEADER*tcp;
    tcp = (TCP_HEADER*)(pkt_content + 14 + 20);
...
/********************** get tcp flags **********************/
QString dataPacketItem::getTcpFlags(){
    TCP_HEADER*tcp;
    tcp = (TCP_HEADER*)(pkt_content + 14 + 20);
...
int dataPacketItem::getTcpOperationRawKind(int offset){
    u_char*tcp;
    tcp = (u_char*)(pkt_content + 14 + 20 + offset + 20);
    return *tcp;
}
bool dataPacketItem::getTcpOperationSACK(int offset,u_char&length,QVector<u_int>&edge){
    u_char*tcp;
    tcp = (u_char*)(pkt_content + 14 + 20 + offset + 20);
    if(*tcp == 5){
        tcp++;
...
bool dataPacketItem::getTcpOperationMSS(int offset, u_short &mss){
    u_char *tcp;
    tcp = (u_char*)(pkt_content + 14 + 20 + offset + 20);
    if(*tcp == 2){
        tcp++;
...
bool dataPacketItem::getTcpOperationSACKP(int offset){
    u_char *tcp;
    tcp = (u_char*)(pkt_content + 14 + 20 + offset + 20);
    if(*tcp == 4)
        return true;
...
bool dataPacketItem::getTcpOperationWSOPT(int offset, u_char &shit){
    u_char *tcp;
    tcp = (u_char*)(pkt_content + 14 + 20 + offset + 20);
    if(*tcp == 3)
    {
...
bool dataPacketItem::getTcpOperationTSPOT(int offset, u_int &value, u_int &reply){
    u_char *tcp;
    tcp = (u_char*)(pkt_content + 14 + 20 + offset + 20);
    if(*tcp == 8){
        tcp++;
...
/* udp */
/********************** get udp source port **********************/
QString dataPacketItem::getUdpSourcePort(){
    UDP_HEADER*udp;
...
QString dataPacketItem::getUdpDestinationPort(){
    UDP_HEADER*udp;
    udp = (UDP_HEADER*)(pkt_content + 20 + 14);
...
/********************** get udp data length **********************/
QString dataPacketItem::getUdpDataLength(){
    UDP_HEADER*udp;
    udp = (UDP_HEADER*)(pkt_content + 20 + 14);
...
/********************** get udp checksum **********************/
QString dataPacketItem::getUdpCheckSum(){
    UDP_HEADER*udp;
    udp = (UDP_HEADER*)(pkt_content + 20 + 14);
...
/********************** get dns domian name **********************/
QString dataPacketItem::getDnsDomainName(int offset){
    char*dns;
...
int dataPacketItem::getDnsAnswersDomain(int offset, QString &name1, u_short &Type, u_short &Class, u_int &ttl, u_short &dataLength,QString&name2){
...
/********************** get dns domain type **********************/
QString dataPacketItem::getDnsDomainType(int type){
    switch (type) {
...
bool dataPacketItem::getisTlsProtocol(int offset){
    char*ssl;
...
void dataPacketItem::getTlsBasicInfo(int offset, u_char &contentType, u_short &version, u_short &length){
    u_char*ssl;
...
void dataPacketItem::getTlsHandshakeType(int offset, u_char &type){
    u_char*ssl;
    ssl = (u_char*)(pkt_content + 14 + 20 + offset);
    type = *ssl;
}
/********************** get tls client hello information **********************/
void dataPacketItem::getTlsClientHelloInfo(int offset, u_char &handShakeType, int &length, u_short &version, QString &random, u_char &sessionIdLength, QString &sessionId,u_short&cipherLength,QVector<u_short> &cipherSuit,u_char& cmLength,QVector<u_char>&CompressionMethod,u_short&extensionLength){
    u_char*ssl;
    ssl = (u_char*)(pkt_content + 14 + 20 + offset);
    handShakeType = *ssl;
    ssl++;
...
void dataPacketItem::getTlsServerHelloInfo(int offset, u_char &handShakeType, int &length, u_short &version, QString& random, u_char &sessionIdLength, QString &sessionId, u_short &cipherSuit, u_char &compressionMethod, u_short &extensionLength){
    u_char*ssl;
    ssl = (u_char*)(pkt_content + 14 + 20 + offset);
    handShakeType = *ssl;
    ssl++;
...
void dataPacketItem::getTlsServerKeyExchange(int offset, u_char &handShakeType, int &length, u_char &curveType, u_short &curveName, u_char &pubLength, QString &pubKey, u_short &sigAlgorithm, u_short &sigLength, QString &sig){
    u_char*ssl;
    ssl = (u_char*)(pkt_content + 14 + 20 + offset);
    handShakeType = *ssl;
    ssl++;
...
/********************** get tls handshake type **********************/
QString dataPacketItem::getTlsHandshakeType(int type){
    switch (type) {
...
QString dataPacketItem::getTlsContentType(int type){
    switch (type) {
...
/********************** get tls version **********************/
QString dataPacketItem::getTlsVersion(int version){
    switch (version) {
...
QString dataPacketItem::getTlsHandshakeCipherSuites(u_short code){
    switch (code) {
...
QString dataPacketItem::getTlsHandshakeCompression(u_char code){
    switch (code) {
    case 0:return "null";
    default:return "";
    }
...
QString dataPacketItem::getTlsHandshakeExtension(u_short type){
    switch (type) {
...
QString dataPacketItem::getTlsHandshakeExtensionECPointFormat(u_char type){
    switch (type) {
...
QString dataPacketItem::getTlsHandshakeExtensionSupportGroup(u_short type){
    switch (type) {
...
QString dataPacketItem::getTlsHadshakeExtensionHash(u_char type){
    switch (type) {
...
QString dataPacketItem::getTlsHadshakeExtensionSignature(u_char type){
    switch (type) {
    case 1:return "RSA";
    case 2:return "DSA";
    case 3:return "ECDSA";
...
u_short dataPacketItem::getTlsExtensionType(int offset){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    return ntohs(*ssl);
}

void dataPacketItem::getTlsExtensionServerName(int offset, u_short &type, u_short &length, u_short &listLength, u_char &nameType, u_short &nameLength, QString &name){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionKeyShare(int offset, u_short &type, u_short &length, u_short &shareLength, u_short &group, u_short &exchangeLength,QString &exchange){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionEcPointFormats(int offset, u_short &type, u_short &length,u_char& ecLength,QVector<u_char> &EC){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionOther(int offset, u_short &type, u_short &length,QString&data){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionSupportGroups(int offset, u_short &type, u_short &length, u_short &groupListLength, QVector<u_short> &group){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionSessionTicket(int offset, u_short &type, u_short &length){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionEncryptThenMac(int offset, u_short &type, u_short &length){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionExtendMasterSecret(int offset, u_short &type, u_short &length){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionSignatureAlgorithms(int offset, u_short &type, u_short &length, u_short &algorithmLength, QVector<u_short> &signatureAlgorithm){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionSupportVersions(int offset, u_short &type, u_short &length, u_char &supportLength, QVector<u_short> &supportVersion){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
void dataPacketItem::getTlsExtensionPadding(int offset, u_short &type, u_short &length,QString& data){
    u_short*ssl;
    ssl = (u_short*)(pkt_content + 14 + 20 + offset);
    type = ntohs(*ssl);
    ssl++;
...
QString dataPacketItem::byteToString(u_char* str, int size)
{
     QString res = "";
     for(int i = 0; i < size ; i++)
     {
