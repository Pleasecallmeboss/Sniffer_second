...
typedef unsigned long u_long;//  4 字节
typedef unsigned int u_int;//  4 字节   其他的长度需要使用1字节的数组
...
+------+--------+-----+------+-------+----------+---------+---------------+--------------+
| hdty |prottype|h_len|pr_len|op_type|source mac|source ip|destination mac|destination ip|
+------+--------+-----+------+-------+----------+---------+---------------+--------------+
*/
typedef struct arp_header{   // 28 byte
    u_short hardware_type;   // hardware type [2 byte]  一般是以太网
    u_short protocol_type;   // protocol [2 byte]        一般是ip
    u_char mac_length;       // MAC address length [1 byte]
    u_char ip_length;        // IP address length [1 byte]
    u_short op_code;         // operation code [2 byte]    请求和答复
...
|                 ack number        4B         |
+----+---------+-------+---------------------+
|head 1B| reserve | flags 1B |     window size  2B   |
+----+---------+-------+---------------------+
